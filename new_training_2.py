...
class CustomGNN(BaseFeaturesExtractor):
    ...
    def forward(self, data):
        ...
        context = self.full_context_nn(th.cat((data["loss"][:,None], data["EdgeFeat(branchflow)"]), -1))
...
# env = openDSSenv34()
    ...
    model = PPO('MultiInputPolicy', env,tensorboard_log="logger/", policy_kwargs=policy_kwargs, verbose=1, n_steps=200, batch_size=100,
            ...
            learning_rate=learning_rate_schedule(0.00001),
                ent_coef=0.05
                ).learn(total_timesteps=80000, n_eval_episodes=1, log_interval=1, tb_log_name="R1_34_env")
